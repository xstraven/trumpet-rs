...
const exerciseKey = document.getElementById("exercise-key");
...
function showResults() {
  ...
  try {
    const analysis = analyze_performance(state.score, state.playedNotes, 50.0, 0.3);
...
if (generateBtn) {
  generateBtn.addEventListener("click", () => {
    ...
    const type = exerciseType ? exerciseType.value : "major_scale";
    const key = exerciseKey ? exerciseKey.value : "C4";
    try {
      const score = generate_exercise(type, key, 100.0);
      state.playing = false;
      state.lastBeat = 0;
      state.score = score;
